...
const LessonCard = ({lesson}) => {
	const {word, pronunciation, part_of_speech, meaning, difficulty} = lesson;
	...
	return (
		<div className="relative max-w-md rounded-2xl bg-white shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-200">
			{/* Difficulty Accent Strip (COLOR TO BE ADDED LATER) */}
			...
			<div className="pl-6 pr-5 py-5 space-y-4">
				...
				<div className="flex items-center justify-between">
					...
					<button className="btn btn-sm rounded-full border-none bg-[#F18701] text-white hover:bg-[#F35B04] transition">When to Say</button>
				</div>
			</div>
		</div>
	);
